...
- **Operator-AI conversational context + broad-overview fix** (implemented from `docs/prompts/operator-ai-conversational-context-and-broad-overview-fix-agent-prompt.md`). A1 follow-up resolution: new `OperatorAiFollowUpResolver` carries the last named entity so a stateless operator turn can answer an anaphoric follow-up ("All active?" after "how many reels?") by prepending the carried entity; gated by a conservative bilingual rung-3 anaphora check (no model call). B: shared `OperatorAiEntityDetection` word-boundary entity matcher; the broad-overview false-positive rule is narrowed and `IsBroadOverviewAsync` gains a flagless single-entity broad-suppress (a single-entity question is never 3B-upgraded to a whole-platform dump). Touches `OperatorAiStatsIntent`, `OperatorAiDecisionHelper`, `ChatHub`, `OperatorAiOptions` (+ `OperatorAiFollowUpResolverTests`, `OperatorAiEntityDetectionTests`, `OperatorAiSingleEntityBroadSuppressTests`). _(Documented here after the fact: this feature's implementation was already present in the working tree and landed in this release alongside the worker-host overview.)_
...
- Trimmed the admin AI-chat waiting elapsed hint `pages.chat.waitingForAiElapsed` to the seconds counter only (e.g. "{{seconds}} s elapsed"), dropping the "— local CPU model; a reply can take 1–5 min. RAM may stay flat while computing." explanatory tail, in all six locales. `{{seconds}}` interpolation preserved; resx-only.
