...
/// <summary>
/// Edge cases for the operator-AI knowledge admin endpoints (§7.2/§8.1/§17.9): reindex coalesces to 409 (RT-17),
/// surfaces 503 on worker/embed failure, and is gated by the global AI switch (RT-13); status returns 503 when the
/// worker is unreachable so the panel can show degraded. Authorization (SUPER_ADMIN, admin face scope) is enforced
/// declaratively by the ManageAllFaces policy on the controller (X5/X6) — the non-operator-denied case is covered at
/// the integration level (PlatformSuperAdminAccessEdge) rather than here, since the controller no longer carries an
/// in-body gate to unit-test.
/// </summary>
public sealed class OperatorAiKnowledgeControllerTests
{
		...
		Mock<IOperatorAiSystemSettingsProvider> settings)
	{
		var controller = new OperatorAiKnowledgeController(
			indexer.Object, status.Object, settings.Object,
	...
	private static (Mock<IOperatorAiKnowledgeIndexer>, Mock<IOperatorAiKnowledgeStatusCache>, Mock<IOperatorAiSystemSettingsProvider>) Mocks(
		bool aiEnabled = true)
	{
		...
		return (new Mock<IOperatorAiKnowledgeIndexer>(), new Mock<IOperatorAiKnowledgeStatusCache>(), settings);
	}
	...
	{
		var (indexer, status, settings) = Mocks(aiEnabled: false);
		var result = await Build(indexer, status, settings).Reindex(default);
	...
	{
		var (indexer, status, settings) = Mocks();
	...
	public async Task Status_returns_503_when_worker_unavailable()
	{
		...
		var result = await Build(indexer, status, settings).Status(default);
		result.Should().BeOfType<ObjectResult>().Which.StatusCode.Should().Be(StatusCodes.Status503ServiceUnavailable);
	}
