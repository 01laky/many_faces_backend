...
- **Admin super-admin self profile:** `AdminMeProfileController` at `/api/admin/me/profile`, password, face role PATCH, email confirm resend; anonymous `GET /api/auth/confirm-email` — see [`docs/guides/admin-superadmin-profile.md`](../docs/guides/admin-superadmin-profile.md). Tests: `dotnet test --filter FullyQualifiedName~AdminMeProfileControllerTests`.
...
- **Operator infra smoke (admin):** read-only **`GET /api/admin/infra/worker-config`**; mail settings **`GET/PUT /api/admin/mail/settings`**, **`POST .../test-smtp`**, smoke **`POST /api/admin/mailer/test-self`**; push **`POST /api/admin/push/test-self`**; search **`GET /api/search/health`** — consumed by **`many_faces_admin`** Settings → **Infrastructure & workers** ([`docs/guides/admin-settings-infrastructure-smoke-tests.md`](../docs/guides/admin-settings-infrastructure-smoke-tests.md), [`docs/guides/admin-mailer-configuration.md`](../docs/guides/admin-mailer-configuration.md)). CI: `node scripts/verify-admin-mail-settings-tests.mjs`.
