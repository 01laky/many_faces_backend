...
- **Operator infra smoke (admin):** read-only **`GET /api/admin/infra/worker-config`**; mail **`POST /api/admin/mailer/test-self`**; push **`POST /api/admin/push/test-self`**; search **`GET /api/search/health`** — consumed by **`many_faces_admin`** Settings → **Infrastructure & workers** ([`docs/guides/admin-settings-infrastructure-smoke-tests.md`](../docs/guides/admin-settings-infrastructure-smoke-tests.md)).
