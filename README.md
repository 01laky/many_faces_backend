...
- **Worker `.proto` source:** **`many_faces_proto`** submodule at monorepo root (`..\..\many_faces_proto\proto\...` from `BeDemo.Api/`), including **`health.proto`** for the AI **HealthService** client. Use **`git submodule update --init --recursive`**.
